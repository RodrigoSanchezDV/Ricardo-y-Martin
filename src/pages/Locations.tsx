...
export default component$(() => {
    ...
    const useResourse = useResource$(async ({ track }) => {
        track(pagina);
        track(searchQuery)
        const response = await apiService.getLocations({page:pagina.value, search: searchQuery.value});
        return response;
    });
    return (
        <Resource
                    ...
                    <div class="mt-8">
                        {
                            ...
                            <div>
                                <div class="flex justify-around">
                                    <h3 class="text-green-500 text-7xl mb-10 text-center">Todas las localizaciones</h3>
                                </div>
                                <div class="flex justify-start">
                                    <h5 class="text-green-500 text-md text-center border border-green-500 mb-2 p-2">Localizaciones encontradas {info.count}</h5>
                                </div>
                            </div>  
                            ...
                            :
                                info && info.count > 0 ?
                                    <div>
                                        <div class="flex justify-around">
                                            <h5 class="text-green-500 text-6xl mb-10 text-center">Localizaciones con {searchQuery.value}</h5>
                                        </div>
                                        <div class="flex justify-start">
                                            <h5 class="text-green-500 text-md text-center border border-green-500 mb-2 p-2">Encontradas {info.count}</h5>
                                        </div>
                                    </div>
                                :
                                <div class="flex flex-wrap justify-center ">
                                    <h3 class="border border-green-500 text-green-500 p-2 text-5x1">No se encontraron localizaciones</h3>
                                </div>
                        }
                        {
                            results.length != 0?
